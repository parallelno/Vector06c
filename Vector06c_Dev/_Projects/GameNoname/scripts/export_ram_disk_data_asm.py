...
def export(source_j, generated_code_dir, segments_paths, 
	...
	asm += "; compressed ram-disk data.\n"
	for name in segments_paths:
		zx0_chunk_paths = segments_paths[name]["zx0_chunk_paths"]
		for zx0_chunk_path in zx0_chunk_paths:
			chunk_name = common.path_to_basename(common.path_to_basename(zx0_chunk_path))
			asm += f"{chunk_name}:\n"
	...
	for bank_j in source_j["banks"]:
		...
		for segment_j in bank_j["segments"]:
			...
			asm += f"; bank{bank_id} addr{addr_s_aligned} [{segment_free_space_s_aligned} free]\n"
			asm += f"; description: {description}\n"
			asm += assets_s + "\n"
